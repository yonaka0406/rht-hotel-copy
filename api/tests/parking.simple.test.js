...
const mockClient = {
    query: async (queryText, values) => {
        ...
        if (queryText.startsWith('INSERT INTO vehicle_categories')) {
            return { rows: [{ id: 1, hotel_id: values[0], name: values[1], capacity_units_required: values[2] }] };
        }
...
(async () => {
    ...
    parkingModel.__setGetPool(mockGetPool);
    reservationsModel.__setGetPool(mockGetPool);

    describe('Parking Management', () => {
        it('should create a vehicle category', async () => {
            const category = {
                hotel_id: 1,
                name: 'Standard Car',
                capacity_units_required: 1,
            };
            const result = await parkingModel.createVehicleCategory('test-request', category);
            assert.deepStrictEqual(result, { id: 1, ...category });
        });
