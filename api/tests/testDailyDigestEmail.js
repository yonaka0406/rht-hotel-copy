...
const generateTestEmailContent = async (requestId, hotelId, date) => {
  ...
  try {
    ...
    // Group logs into categories: added, edited, deleted
    const groupedLogs = {
      added: [],
      edited: [],
      deleted: []
    ...
    return {
      ...
      text: `日次予約ログダイジェスト - ${hotel.name} - ${formattedDate}\n\n` +
            `本日追加された予約:\n${groupedLogs.added.map(log => `予約ID: ${log.record_id}, ホテル名: ${log.hotel_name}`).join('\n')}\n\n` +
            `本日編集された予約:\n${groupedLogs.edited.map(log => `予約ID: ${log.record_id}, ホテル名: ${log.hotel_name}`).join('\n')}\n\n` +
            `本日削除された予約:\n${groupedLogs.deleted.map(log => `予約ID: ${log.record_id}, ホテル名: ${log.hotel_name}`).join('\n')}`, // Simple text fallback
      html: htmlContent
    };
