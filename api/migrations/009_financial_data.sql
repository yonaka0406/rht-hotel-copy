...
CREATE TABLE IF NOT EXISTS du_forecast_entries (
    id SERIAL PRIMARY KEY,
    hotel_id INT REFERENCES hotels(id), -- Nullable for global/office departments
    department_name VARCHAR(24), -- Accounting department name (部門)
    ...
    CONSTRAINT uq_hotel_dept_month_account_sub_forecast UNIQUE NULLS NOT DISTINCT (hotel_id, department_name, month, account_name, sub_account_name)
);
...
CREATE INDEX IF NOT EXISTS idx_du_forecast_entries_dept_name ON du_forecast_entries(department_name);
CREATE INDEX IF NOT EXISTS idx_du_forecast_entries_hotel_dept_month_name_sub ON du_forecast_entries(COALESCE(hotel_id, 0), COALESCE(department_name, ''), month, account_name, COALESCE(sub_account_name, ''));
