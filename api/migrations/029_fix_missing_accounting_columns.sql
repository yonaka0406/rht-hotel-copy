-- Migration: 029_fix_missing_accounting_columns.sql
-- Description: Add missing columns to accounting tables and update views

-- 1. Add default_account_type to acc_management_groups if missing
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='acc_management_groups' AND column_name='default_account_type') THEN
        ...
        UPDATE acc_management_groups SET default_account_type = 'debit' WHERE id IN (2, 3, 4, 5, 7, 9, 10);
    END IF;
END $$;

-- 2. Add account_type to acc_account_codes if missing
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='acc_account_codes' AND column_name='account_type') THEN
        ALTER TABLE acc_account_codes ADD COLUMN account_type VARCHAR(10) CHECK (account_type IN ('debit', 'credit'));

        -- Sync account_type from management group if already set
