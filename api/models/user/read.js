...
const selectAllUsers = async (requestId, dbClient = null) => {
    ...
    const query = 'SELECT users.id, users.email, users.name, users.status_id, users.role_id, users.auth_provider, users.created_at, user_roles.role_name, user_roles.permissions FROM users, user_roles WHERE users.role_id = user_roles.id ORDER BY status_id, role_id, email, id ASC';
