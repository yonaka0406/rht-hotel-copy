<template>
    <div class="p-4">
        <Panel header="ユーザー 新規 & 編集">
            <div>
                <div class="text-left">
                    <Button label="新規ユーザー" icon="pi pi-user-plus" class="p-button-success mb-4" @click="createUser" />
                </div>
                ...
                <Dialog v-model="createUserDialog" v-if="createUserDialog" header="新規ユーザー登録" :visible="createUserDialog"
                    ...
                    <form @submit.prevent="submitNewUser">
                        ...
                        <div class="mb-3 mt-6 text-center gap-2">
                            <!-- Submit Button -->
                            <Button type="submit" label="新規ユーザー" class="p-button-success" icon="pi pi-check"
                                severity="success" />
                            <!-- Cancel Button -->
                            <Button type="button" label="キャンセル" class="p-button-text p-button-secondary"
                                icon="pi pi-times" severity="danger" @click="cancelCreateDialog()" />
                        </div>
                            ...
                            <AccordionHeader>
                                <div class="flex gap-4">
                                    ...
                                    <div class="flex items-center">
                                        <Badge v-if="activeUsersCount > 0" class="ml-2" :value="activeUsersCount"
                                            severity="success" />
                                    </div>
                                </div>
                            </AccordionHeader>
                            <AccordionContent>
                                <DataTable v-model:filters="filters" :value="activeUsers" removableSort paginator
                                    :rows="10" class="p-datatable-sm" dataKey="id" filterDisplay="row"
                                    :loading="loading" :globalFilterFields="['email', 'role_name']" scrollable
                                    responsive>
                                    <template #header>
                        ...
                        <AccordionPanel value="2">
                            <AccordionHeader>
                                <div class="flex gap-4">
                                    ...
                                    <div class="flex items-center">
                                        <Badge v-if="deactivatedUsersCount > 0" class="ml-2"
                                            :value="deactivatedUsersCount" severity="danger" />
                                    </div>
                                </div>
                            </AccordionHeader>
                            <AccordionContent>
                                <DataTable v-model:filters="filters" :value="deactivatedUsers" removableSort paginator
                                    :rows="10" class="p-datatable-sm" dataKey="id" filterDisplay="row"
                                    :loading="loading" :globalFilterFields="['email', 'role_name']" scrollable
                                    responsive>
                                    <template #header>
                ...
                <Dialog v-model="editUserDialog" v-if="editUserDialog" header="ユーザー編集" :visible="editUserDialog"
                    ...
                    <form @submit.prevent="submitEditUser">
                        ...
                        <div class="field mt-6 text-center">
                            <ToggleButton v-model="statusToggle" onLabel="有効" offLabel="無効" onIcon="pi pi-lock-open"
                                offIcon="pi pi-lock" class="w-36" aria-label="User Status Toggle" />
                        </div>

                        <!-- Reset Password -->
                        <div v-if="currentUser.auth_provider === 'local'" class="field mt-6 text-center">
                            <Button label="パスワードリセット依頼" class="p-button-info" @click="sendResetPasswordEmail" />
                        </div>
                        ...
                        <div class="mb-3 mt-6 text-center gap-2">
                            <!-- Edit Button -->
                            <Button type="submit" label="更新" class="p-button-warning" icon="pi pi-pencil"
                                severity="warning" />
                            <!-- Cancel Button -->
                            <Button type="button" label="キャンセル" class="p-button-text p-button-secondary"
                                icon="pi pi-times" severity="danger" @click="cancelEditDialog" />
                        </div>
                    </form>
                </Dialog>
