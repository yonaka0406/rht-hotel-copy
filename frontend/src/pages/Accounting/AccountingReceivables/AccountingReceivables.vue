        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              サブアカウント（クライアント）別の売掛金残高確認とフォローアップ
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
        ...
        <div class="p-8 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div class="flex items-center gap-4 w-full md:w-auto">
            ...
            <button @click="loadBalances" :disabled="isLoading" class="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-400 hover:text-violet-600 transition-all cursor-pointer shadow-sm disabled:opacity-50">
        ...
        <div class="p-0">
          ...
          >
            ...
            <Column header="操作" class="text-center w-48">
              <template #body="slotProps">
                <div class="flex items-center justify-center gap-2">
                  <button 
                    @click="openLinkDialog(slotProps.data)"
                    class="flex items-center gap-2 px-3 py-1.5 bg-violet-50 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 rounded-lg text-xs font-bold hover:bg-violet-100 transition-all cursor-pointer"
                    v-tooltip.top="'CRMクライアントを検索してアクションを追加'"
                  >
...
const { balances, searchResults, isLoading: isModelLoading, fetchBalances, searchClients } = useAccountingReceivables();
