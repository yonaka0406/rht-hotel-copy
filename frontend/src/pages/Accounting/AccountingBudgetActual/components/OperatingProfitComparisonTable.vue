...
<template>
    <div class="mt-8">
        <div class="flex items-center gap-2 mb-4">
            <i class="pi pi-money-bill text-violet-600 dark:text-violet-400"></i>
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">施設別営業利益予実</h3>
        </div>
                ...
                <table class="w-full text-sm text-left border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50">
                            ...
                            <th class="py-3 px-6 font-bold text-slate-900 dark:text-white text-right">予算 営業利益</th>
                            <th class="py-3 px-6 font-bold text-slate-900 dark:text-white text-right">実績 営業利益</th>
                            <th class="py-3 px-6 font-bold text-slate-900 dark:text-white text-right">予実差</th>
                        </tr>
                    </thead>
                    ...
                    <tbody v-else-if="hotelRows.length > 0">
                        <tr v-for="row in hotelRows" :key="row.hotelId"
                            ...
                            <td class="py-4 px-6 text-right text-slate-900 dark:text-white font-mono">{{ formatNumber(row.budget) }}</td>
                            <td class="py-4 px-6 text-right text-slate-900 dark:text-white font-mono">{{ formatNumber(row.actual) }}</td>
