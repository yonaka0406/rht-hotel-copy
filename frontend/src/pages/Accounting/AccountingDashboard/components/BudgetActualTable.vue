...
const formatNumber = (val) => {
    ...
    // Round to nearest integer for currency display as requested in example
...
const rows = computed(() => {
    ...
    const createRow = (label, getVal, isProfit = false) => {
        ...
        // Ratio calculation (relative to Revenue)
        const aRev = findAmount(actual, 1);
        const bRev = findAmount(budget, 1);

        // For common accounting reporting, we show expense ratios as positive
