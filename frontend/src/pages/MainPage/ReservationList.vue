            ...
            <template #content>
                <div class="flex justify-between items-center flex-wrap gap-2">
                    ...
                    <div class="flex items-center flex-wrap gap-2 flex-grow">
                        <FloatLabel>
                            <Select v-model="searchType" :options="searchTypeOptions" optionLabel="label" class="mr-2"
                                fluid />
                            <label>検索範囲:</label>
                        </FloatLabel>
                        <FloatLabel>
                            <DatePicker v-model="startDateFilter" dateFormat="yy-mm-dd" :selectOtherMonths="true"
                                class="mr-2" fluid />
                            <label>{{ dateRangeLabel.start }}:</label>
                        </FloatLabel>
                        <FloatLabel>
                            <DatePicker v-model="endDateFilter" dateFormat="yy-mm-dd"
                                :placeholder="`${dateRangeLabel.end}を選択`" :selectOtherMonths="true" class="mr-2"
                                fluid />
                            <label>{{ dateRangeLabel.end }}:</label>
                        </FloatLabel>
                        <Button label="適用" class="mr-2" size="small" @click="applyDateFilters"
                    ...
                    <div class="flex justify-end gap-2 flex-shrink-0">
                        <Button label="全フィルタークリア" icon="pi pi-filter-slash" severity="secondary" size="small"
                            @click="clearAllFilters" v-tooltip.bottom="'全てのフィルターをリセットします'" />
                        <SplitButton label="エクスポート" icon="pi pi-file-export" severity="help" size="small"
                            @click="splitButtonExportReservations" :model="exportOptions" />
                    </div>
                </div>
            </template>
        </Card>
        <div class="table-container">
            <DataTable v-model:filters="filters" v-model:selection="selectedReservations" filterDisplay="row"
                :value="filteredReservations" :loading="tableLoading" size="small" :paginator="true" :rows="25"
                :rowsPerPageOptions="[5, 10, 25, 50, 100]" dataKey="id" stripedRows @row-dblclick="openDrawer"
                removableSort v-model:expandedRows="expandedRows" :rowExpansion="true">
                <template #header>
                ...
                <Column header="詳細" style="width: 1%;">
                    <template #body="slotProps">
                        <button @click="toggleRowExpansion(slotProps.data)"
