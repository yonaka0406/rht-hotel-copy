<template>
  <Dialog :visible="visible" @update:visible="handleClose" :header="`順番待ちリスト登録 - ${initialHotelName}`" :closable="true"
    ...
    <div v-if="currentStep === '1'" class="grid grid-cols-2 gap-x-4 gap-y-6 pt-6">
      ...
      <div class="col-span-2 mb-2">
        <FloatLabel>
          <ClientAutoCompleteWithStore v-model="internalForm.client_name_waitlist"
            @option-select="onClientSelectForWaitlist" :forceSelection="false" label="顧客名（検索または新規入力）" />
      ...
      <div class="col-span-1" v-if="!isClientSelectedForWaitlist">
        <SelectButton v-model="internalForm.client_legal_or_natural_person_waitlist" :options="personTypeOptions"
          option-label="label" option-value="value" fluid />
      </div>
      <div class="col-span-1"
        v-if="!isClientSelectedForWaitlist && internalForm.client_legal_or_natural_person_waitlist === 'natural'">
        <div class="flex gap-3">
          <div v-for="option in genderOptions" :key="option.value" class="flex items-center gap-2">
            <RadioButton v-model="internalForm.client_gender_waitlist"
              :inputId="`dialog_waitlist_gender_${option.value}`" :name="`dialog_waitlist_gender`"
              :value="option.value" />
      ...
      <div class="col-span-1">
        <FloatLabel>
          <InputText v-model="internalForm.contact_email" type="email" fluid
      ...
      <div class="col-span-1">
        <FloatLabel>
          <InputText v-model="internalForm.contact_phone" type="tel" fluid
      ...
      <div class="col-span-2">
        ...
        <div class="flex gap-4">
          <div class="flex items-center">
            <RadioButton v-model="internalForm.communication_preference" inputId="dialog_comm_email_waitlist"
              name="dialog_comm_pref" value="email" />
            <label for="dialog_comm_email_waitlist" class="ml-2">メール</label>
          </div>
          <div class="flex items-center">
            <RadioButton v-model="internalForm.communication_preference" inputId="dialog_comm_phone_waitlist"
              name="dialog_comm_pref" value="phone" />
    ...
    <div v-if="currentStep === '2'" class="grid grid-cols-2 gap-x-4 gap-y-6 pt-6">
      ...
      <div class="col-span-2">
        ...
        <div class="flex gap-4">
          <div v-for="option in smokingOptionsDialog" :key="option.value" class="flex items-center">
            <RadioButton v-model="selectedSmokingPreferenceDialog" :inputId="`dialog_smoking_${option.value}`"
              name="dialogSmokingPreference" :value="option.value" @change="updateFormSmokingPreference" />
      ...
      <div class="col-span-1">
        <FloatLabel>
          <Select v-model="internalForm.room_type_id" :options="filteredRoomTypes" optionLabel="room_type_name"
            optionValue="room_type_id" fluid :showClear="true" />
          <label>希望部屋タイプ（任意）</label>
        </FloatLabel>
      </div>
      <div class="col-span-1">
        <FloatLabel>
          <InputNumber v-model="internalForm.number_of_rooms" :min="1" fluid required />
          <label>部屋数 *</label>
        </FloatLabel>
      </div>
      <div class="col-span-1">
        <FloatLabel>
          <InputNumber v-model="internalForm.number_of_guests" :min="1" fluid />
          <label>合計人数</label>
        </FloatLabel>
      </div>
      <div class="col-span-1">
        <FloatLabel>
          <InputText v-model="internalForm.requested_check_in_date" type="date" fluid @change="onCheckInDateChange" />
      ...
      <div class="col-span-1">
        <FloatLabel>
          <InputText v-model="internalForm.requested_check_out_date" type="date" :min="minCheckOutDate" fluid
    ...
    <div class="flex justify-between mt-6">
      <Button v-if="currentStep === '2'" label="前へ" severity="secondary" icon="pi pi-arrow-left" @click="goToStep(1)" />
      <div v-else></div>

      <Button v-if="currentStep === '1'" label="次へ" icon="pi pi-arrow-right" iconPos="right" @click="goToStep(2)" />
    </div>

    <template #footer>
      <Button label="閉じる" icon="pi pi-times" @click="handleClose" class="p-button-text p-button-danger p-button-sm" />
      <Button v-if="currentStep === '2' && isFormValid" label="登録" icon="pi pi-check" @click="handleSubmit"
        :loading="isLoading" class="p-button-text p-button-success p-button-sm"
