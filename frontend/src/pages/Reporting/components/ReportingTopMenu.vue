...
<script setup>
    ...
    const localSelectedHotels = ref([]); // Initialize with prop or default // Modified to init as []
    ...
    // Computed property for single hotel selection, mapping to/from localSelectedHotels array
    const singleSelectedHotelId = computed({
      get: () => (localSelectedHotels.value?.length > 0) ? localSelectedHotels.value[0] : null,
      set: (val) => { localSelectedHotels.value = (val === null || val === undefined) ? [] : [val]; }
    });
    ...
    function onInternalHotelSelectionChange() {
      ...
      // v-model (on localSelectedHotels or singleSelectedHotelId) would have already updated localSelectedHotels.value.
      emit('hotel-change', localSelectedHotels.value, hotels.value);
    }
    ...
    function onInternalReportTypeChange(event) { 
        ...
        if (newReportType === 'activeReservationsChange') {
            localSelectedHotels.value = [0]; // Default to "All Facilities"
            ...
            if (hotels.value && hotels.value.length > 0) {
                if (!localSelectedHotels.value || localSelectedHotels.value.length === 0 ||
                    (localSelectedHotels.value.length === 1 && localSelectedHotels.value[0] === 0) || // Coming from "All Facilities"
                    localSelectedHotels.value.length > 1) { // Or if multiple were selected
                    localSelectedHotels.value = [hotels.value[0].id];
                }
                // If a single, valid hotel was already selected, keep it.
            } else {
                localSelectedHotels.value = []; // No hotels available
            }
            ...
            if (localSelectedHotels.value && localSelectedHotels.value.length === 1 && localSelectedHotels.value[0] === 0) {
                localSelectedHotels.value = hotels.value ? hotels.value.map(h => h.id) : []; // Select all hotels
            } else if (!localSelectedHotels.value || localSelectedHotels.value.length === 0) {
                // If no hotels were selected, default to all hotels for multi-select reports
                localSelectedHotels.value = hotels.value ? hotels.value.map(h => h.id) : [];
            }
        }
        // Note: If switching from a multi-hotel selection to another multi-hotel report (e.g. monthlySummary to another),
        // localSelectedHotels generally remains as is, which is usually the desired behavior.

        emit('report-type-change', newReportType);
        // Ensure hotel-change is emitted after localSelectedHotels is finalized for the new report type
        emit('hotel-change', localSelectedHotels.value, hotels.value);
    }
    ...
    watch(() => props.selectedHotels, (newValue) => {
      localSelectedHotels.value = newValue ? [...newValue] : [];
    ...
    watch(showSingleHotelSelect, (newValue, oldValue) => {
        ...
        // console.log('[ReportingTopMenu] localSelectedHotels.value when showSingleHotelSelect changed:', localSelectedHotels.value);
        ...
        if (oldValue === true && newValue === false) {
            ...
            if (localSelectedHotels.value && localSelectedHotels.value.length === 1 && localSelectedHotels.value[0] === 0) {
                localSelectedHotels.value = hotels.value ? hotels.value.map(h => h.id) : []; // Select all hotels
                emit('hotel-change', localSelectedHotels.value, hotels.value); // Emit the change to parent
            }
        }
    });
    ...
    onMounted(async () => {
      ...
      // props.selectedHotels watcher with immediate:true has already run and populated localSelectedHotels.value.
      // Now, adjust localSelectedHotels.value based on the effective selectedReportType.
      if (selectedReportType.value === 'activeReservationsChange') {
        if (localSelectedHotels.value && localSelectedHotels.value.length > 0) {
          // Ensure it's a single selection, typically [0] for "All Facilities"
          localSelectedHotels.value = [localSelectedHotels.value[0]];
        } else {
          localSelectedHotels.value = [0]; // Default to "All Facilities"
        }
        ...
        if (hotels.value && hotels.value.length > 0) {
          if (!localSelectedHotels.value || localSelectedHotels.value.length === 0 || localSelectedHotels.value.length > 1) {
            localSelectedHotels.value = [hotels.value[0].id]; // Select the first hotel
          }
        ...
        if ((!localSelectedHotels.value || localSelectedHotels.value.length === 0) && hotels.value && hotels.value.length > 0) {
          localSelectedHotels.value = hotels.value.map(h => h.id); // Default to all hotels
        }
      }
      ...
      emit('hotel-change', localSelectedHotels.value, hotels.value); // localSelectedHotels.value is an array
