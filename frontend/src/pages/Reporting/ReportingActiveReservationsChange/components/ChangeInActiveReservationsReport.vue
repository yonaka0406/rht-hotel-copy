<template>
    <div class="p-4">
            ...
            <Card>
                <template #title>
                    <div class="flex items-center">
                        ...
                        <span>前日合計 ({{ previousDateLabel }})</span>
                    </div>
                </template>
                <template #content>
                    <p class="text-sm text-gray-600 mb-1">在庫数合計:</p>
                    <p class="text-3xl font-bold text-blue-600">{{
                        formatNumberWithThousandsSeparator(totalPreviousDayEnd) }}</p>
                </template>
            </Card>
            <Card>
                <template #title>
                    <div class="flex items-center">
                        ...
                        <span>指定日合計 ({{ comparisonDate }})</span>
                    </div>
                </template>
                <template #content>
                    <p class="text-sm text-gray-600 mb-1">在庫数合計:</p>
                    <p class="text-3xl font-bold text-green-600">{{
                        formatNumberWithThousandsSeparator(totalSnapshotDayEnd) }}</p>
                </template>
            </Card>
        </div>
        ...
        <div v-if="reportData && reportData.length > 0" class="mt-8">
            <Card>
                <template #header>
                    <h4 class="text-lg font-semibold mb-3">詳細データ (指定日: {{ comparisonDate }})</h4>
                </template>
                <template #content>
                    <DataTable :value="sortedReportDataForTable" responsiveLayout="scroll" rowGroupMode="subheader"
                        groupRowsBy="yearMonth" expandableRowGroups v-model:expandedRowGroups="expandedRowGroups">
                        <template #groupheader="slotProps">
                            <div class="p-datatable-group-header-content flex items-center p-2 cursor-pointer w-full"
                                @click="toggleRowGroup(slotProps.data.yearMonth)">
                                <!-- Column 1: Month and Toggle Icon -->
                                <div class="flex items-center" style="width: 25%;">
                                    <i :class="expandedRowGroups.includes(slotProps.data.yearMonth) ? 'pi pi-chevron-down' : 'pi pi-chevron-right'"
                                        class="mr-2 text-blue-500 text-sm"></i>
                                ...
                                <div style="width: 25%; text-align: right; padding-right: 0.5rem;">
                                    <span class="text-xs text-gray-500">前日合計: <span
                                            class="font-semibold text-gray-700">{{
                                                formatNumberWithThousandsSeparator(calculateGroupTotals(slotProps.data.yearMonth).prevDayTotal)
                                            }}</span></span>
                                </div>

                                <!-- Column 3: Snapshot Day Total (aligned right) -->
                                <div style="width: 25%; text-align: right; padding-right: 0.5rem;">
                                    <span class="text-xs text-gray-500">指定日合計: <span
                                            class="font-semibold text-gray-700">{{
                                                formatNumberWithThousandsSeparator(calculateGroupTotals(slotProps.data.yearMonth).snapshotDayTotal)
                                            }}</span></span>
                                </div>
                                ...
                                <div style="width: 25%; text-align: right; padding-right: 0.5rem;">
                                    <span class="text-xs text-gray-500">日次差異合計: <span class="font-semibold"
                                            :class="{ 'text-red-500': calculateGroupTotals(slotProps.data.yearMonth).diffTotal < 0, 'text-green-500': calculateGroupTotals(slotProps.data.yearMonth).diffTotal >= 0 }">{{
                                                formatNumberWithThousandsSeparator(calculateGroupTotals(slotProps.data.yearMonth).diffTotal)
                                            }}</span></span>
                                </div>
                            </div>
                        </template>

                        <Column field="inventory_date_formatted" header="日付 (現地時間)" :sortable="true" style="flex: 1;">
                        </Column>
                        <Column field="count_as_of_previous_day_end" header="前日在庫" :sortable="true"
                            style="flex: 1; text-align: right;">
                            <template #body="columnSlotProps">
                                {{ columnSlotProps.data.count_as_of_previous_day_end }}
                            </template>
                        </Column>
                        <Column field="count_as_of_snapshot_day_end" header="当日在庫" :sortable="true"
                            style="flex: 1; text-align: right;">
                            <template #body="columnSlotProps">
...
const { fetchActiveReservationsChange, fetchLatestDailyReportDate } = useReportStore();
...
const previousDateLabel = computed(() => {
    ...
    const d = new Date(comparisonDate.value);
...
const fetchReportData = async () => {
    ...
    try {
        ...
        const latestDateInDb = await fetchLatestDailyReportDate();

        // Default to the latest date in the database, but fallback to today if not available
        let targetDate = latestDateInDb || todayDateString;

        // If the latest date is the same as current date, the comparison becomes "wrong" according to the user.
        // We should use the previous day as the snapshot date in this case to ensure we compare two complete days or the most relevant historical snapshot.
        ...
        // console.log(`Fetching active reservations change for hotel: ${props.hotelId}, comparison date: ${targetDate}, trigger: ${props.triggerFetch}`);

        // Call the store function with hotelId and the selected comparison date
        const data = await fetchActiveReservationsChange(props.hotelId, targetDate);
