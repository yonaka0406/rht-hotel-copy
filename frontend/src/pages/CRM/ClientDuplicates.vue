                                ...
                                <ul class="p-2">
                                    <li v-for="duplicate in data.duplicates" :key="duplicate.id" class="mb-2">
                                        <div class="grid grid-cols-2 flex flex-col gap-1">
                                            ...
                                            <div class="flex items-center gap-2" v-tooltip="'法人・個人'">
                                                <i class="pi pi-users text-xs" />
                                                <span class="text-xs">{{ duplicate.legal_or_natural_person === 'natural'
                                                    ? '個人' : '法人' }}</span>
                                            </div>
                                            <div class="flex items-center gap-2" v-tooltip="'性別'">
                                                <i class="pi pi-venus text-xs" /><i class="pi pi-mars text-xs" />
...
const clientsWithPotentialDuplicates = computed(() => {
    // If clients is not an array or is empty, return an empty array to avoid errors.
...
const formatDate = (date) => {
    if (!(date instanceof Date) || isNaN(date.getTime())) {
        // This was throwing an error, which can be disruptive. Changed to console.error and return a fallback string.
...
watch(clientsIsLoading, (isLoading) => {
    // This logic hides the drawer when loading starts.
    if (isLoading) {
