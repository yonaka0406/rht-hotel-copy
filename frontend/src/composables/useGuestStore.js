...
export function useGuestStore() {
    ...
    const generateGroupGuestListPDF = async (hotelId, reservationId, guestData) => {
        isGenerating.value = true;
        try {
            const authToken = localStorage.getItem('authToken');
            const url = `/api/guests/generate-group-list/${hotelId}/${reservationId}`;
            ...
            if (!response.ok) {
                throw new Error('Failed to generate group guest list PDF');
            }
            ...
            const disposition = response.headers.get('content-disposition');
            let filename = `guest_list_group_${reservationId}.pdf`;
            if (disposition && disposition.indexOf('attachment') !== -1) {
                const filenameRegex = /filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/;
                const matches = filenameRegex.exec(disposition);
                if (matches != null && matches[1]) {
